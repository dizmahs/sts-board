...
function ViewDeemed(ScrollFlag){
	if(GetObject("DEEMED_LAYER") == null){
		...
		if(DeemedLayer.style.width <= 0){
			DeemedLayer.style.width = "100%";
		}
		
		if(DeemedLayer.style.height <= 0){
			DeemedLayer.style.height = "100%";
		}
		...
		if(ScrollFlag != null && ScrollFlag == false){
			...
			else if(document.documentElement.style.overflow != null){
				document.documentElement.style.overflow = "hidden";
			}
			else if(document.documentElement.style.overflowY != null){
...
function ObjFloatValue(ObjectId){
	var Obj = GetObject(ObjectId);
	var FloatValue = 0;
	
	if(Obj != null && Obj.value.length > 0){
		try{
			FloatValue = parseFloat(Obj.value.replace(/[^0-9^.\-]/g, ''), 10);
		}catch(e){
			FloatValue = 0;
		}
	}
...
function CheckPassword(TargetStr){
	var ChkUpperCharStr = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
	var ChkLowerCharStr = "abcdefghijklmnopqrstuvwxyz";
	var ChkSpecCharStr = "~!@#()-_+";
	var ChkNumberStr = "0123456789";
	...
	var SpecCharFlag = false;
	var NumberFlag = false;
	
	for(var idx = 0; idx < TargetStr.length; idx++){
		ChkChar = TargetStr.substring(idx, idx + 1);
		
		if(ChkUpperCharStr.indexOf(ChkChar) >= 0){
			UpperCharFlag = true;
		}
		else if(ChkLowerCharStr.indexOf(ChkChar) >= 0){
			LowerCharFlag = true;
		}
		else if(ChkSpecCharStr.indexOf(ChkChar) >= 0){
			SpecCharFlag = true;
		}
		else if(ChkNumberStr.indexOf(ChkChar) >= 0){
			NumberFlag = true;
		}
		else{
			alert("사용할 수 없는 문자입니다.\n\n(영문대소문자, 숫자, 특수문자(" + ChkSpecCharStr + ")만 사용가능합니다.)");
			CheckFlag = true;
			break;
		}
	}
	
	if(TargetStr.length > 0 && TargetStr.length < 8){
		alert("8자 이상 사용가능합니다.");
		CheckFlag = true;
	}
	
	if(!CheckFlag && TargetStr.length >= 8){
		var PatternCnt = 0;
		...
		if(TargetStr.length < 10 && PatternCnt < 3){
			alert("영문대문자, 영문소문자, 숫자, 특수문자(" + ChkSpecCharStr + ") 중 3종류 이상 입력해야합니다.");
			CheckFlag = true;
		}
		else if(PatternCnt < 2){
			alert("영문대문자, 영문소문자, 숫자, 특수문자(" + ChkSpecCharStr + ") 중 2종류 이상 입력해야합니다.");
			CheckFlag = true;
		}
	}
...
function PopCalendar(ObjectName, PrevYears, NextYears){
	...
	var url = "/SejongTotalManage/comm/Calendar.jsp?CALENDAR_DATE=" + CalendarDate + "&OBJECT_NAME=" + ObjectName + "&PREV_YEARS=" + PrevYears + "&NEXT_YEARS=" + NextYears;
...
function PopZip(ZipCdObjectName, AddrObjectName){
	var url = "/SejongTotalManage/comm/StreetCode.jsp?ZIP_CD_OBJECT_NAME=" + ZipCdObjectName + "&ADDR_OBJECT_NAME=" + AddrObjectName;
...
function FileUpload(FileType, FileIdx, MultipleFlag){
	...
	var url = "/SejongTotalManage/comm/FileUpload.jsp?FILE_TYPE=" + FileType + "&FILE_IDX=" + FileIdx + "&MULTIPLE_FLAG=" + MultipleFlag;
...
function PrintLayout(LayoutWidth, LayoutHeight){
	if(LayoutWidth == null){
		LayoutWidth = 1100;
	}
	...
	var url = "/SejongTotalManage/comm/PrintLayout.jsp";
	window.open(url, "PrintLayout", "width=" + LayoutWidth + ", height=" + LayoutHeight + ", toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no, directories=no, status=no");
}

function AuthGPin(ProcessType, ProcessUrl){
	if(ProcessType == null){
		ProcessType = "";
	}
	...
	var url = "https://totalmanage.sejongpac.or.kr/SejongTotalManage/GPin/AuthRequest.jsp?PROCESS_TYPE=" + ProcessType + "&PROCESS_URL=" + encodeURI(ProcessUrl);
	var CheckObj = window.open(url, "AuthGPin", "width=450, height=560, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no");
